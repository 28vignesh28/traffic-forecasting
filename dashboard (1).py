...
def get_congestion_info(speed_kmh):
...
def generate_ai_insights(pred_speed, congestion_label, horizon_label, actual_speed=None, trend_speeds=None):
    ...
    if actual_speed is not None:
        ...
        if abs(change) > 5:
            ...
            insights.append(f"🔮 Model predicts a **{abs(change):.1f} km/h {direction}** compared to the actual recorded speed at this timestamp.")
...
# Inverse transform to mph then convert to km/h
preds_mph = preds_np * scaler_std + scaler_mean
preds_kmh = preds_mph * 1.60934  # mph → km/h

# Target sensor predicted speeds (all horizons)
sensor_preds_kmh = preds_kmh[:, sel_sensor]  # [12]
pred_speed = float(sensor_preds_kmh[sel_horizon_idx])
...
if found_in_dataset:
    ...
    if future_ts in traffic_df.index:
        ...
        actual_speed = actual_mph * 1.60934

# Also get actual historical speeds for the past window
actual_history_kmh = None
if found_in_dataset:
    ...
    ].iloc[:, sel_sensor].values * 1.60934
    actual_history_kmh = history
...
insights = generate_ai_insights(
    ...
    actual_speed=actual_speed, trend_speeds=sensor_preds_kmh
)
...
with col1:
    ...
    <div class="metric-card">
        ...
        <div class="metric-value">{pred_speed:.1f} <span style="font-size:0.9rem;color:#64748b;">km/h</span></div>
...
with chart_col:
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="<b>+%{x} min</b><br>Predicted: %{y:.1f} km/h<extra></extra>",
    ))

    # Actual future line (if data exists)
    if found_in_dataset:
        ...
        for i in range(12):
            ...
            if ft in traffic_df.index:
                actual_future_speeds.append(float(traffic_df.loc[ft].iloc[sel_sensor]) * 1.60934)
        ...
        if actual_filtered:
            ...
            fig.add_trace(go.Scatter(
                ...
                hovertemplate="<b>+%{x} min</b><br>Actual: %{y:.1f} km/h<extra></extra>",
            ))

    # Mark selected horizon
    fig.add_vline(
        x=future_minutes[sel_horizon_idx],
        line_dash="dash",
        line_color="#6366f1",
        opacity=0.6,
        annotation_text=f"Selected: {sel_horizon_label}",
        annotation_font_color="#a5b4fc",
        annotation_font_size=11,
    )

    # Speed zone bands
    fig.add_hrect(y0=60, y1=max(120, max(sensor_preds_kmh) + 10), fillcolor="rgba(16,185,129,0.06)", line_width=0,
                  annotation_text="Free Flow", annotation_position="top right",
                  annotation_font=dict(color="#6ee7b7", size=10))
    fig.add_hrect(y0=40, y1=60, fillcolor="rgba(245,158,11,0.06)", line_width=0,
                  annotation_text="Moderate", annotation_position="top right",
                  annotation_font=dict(color="#fbbf24", size=10))
    fig.add_hrect(y0=0, y1=40, fillcolor="rgba(244,63,94,0.06)", line_width=0,
                  annotation_text="Congested", annotation_position="top right",
                  annotation_font=dict(color="#fb7185", size=10))
    ...
    fig.update_layout(
        ...
        title=dict(
            text=f"Traffic Speed Forecast — Sensor #{sel_sensor}",
            font=dict(size=16, color="#e2e8f0"),
        ),
        xaxis=dict(
            title="Minutes from Selected Time",
            gridcolor="rgba(148,163,184,0.1)",
            tickvals=future_minutes,
            ticktext=future_labels,
        ),
        yaxis=dict(
            title="Speed (km/h)",
            gridcolor="rgba(148,163,184,0.1)",
            range=[0, max(120, max(sensor_preds_kmh) + 15)],
        ),
        legend=dict(
            orientation="h",
            yanchor="bottom",
...
with info_col:
    ...
    if actual_speed is not None:
        change = pred_speed - actual_speed
        if change > 0:
            change_html = f'<span class="change-up">Increase (+{change:.1f} km/h) ↑</span>'
        elif change < 0:
            change_html = f'<span class="change-down">Decrease ({change:.1f} km/h) ↓</span>'
        else:
            change_html = '<span class="change-neutral">No Change (0.0 km/h) →</span>'

        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-label">Actual Speed</div>
            <div class="metric-value" style="font-size:1.4rem;">{actual_speed:.1f} <span style="font-size:0.8rem;color:#64748b;">km/h</span></div>
        </div>
        <div class="metric-card">
            <div class="metric-label">Predicted Speed</div>
            <div class="metric-value" style="font-size:1.4rem;">{pred_speed:.1f} <span style="font-size:0.8rem;color:#64748b;">km/h</span></div>
        </div>
        <div class="metric-card">

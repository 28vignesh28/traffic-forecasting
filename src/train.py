...
def train_unified_model(model_name, config_path="src/config.yaml"):
    ...
    logger.info(
        f"[{model_name}] Data Loaded: {nodes} Nodes, {features} Features (Traffic + Context)"
    )
    ...
    if model_name == "CADGT":
        ...
        model = CADGT(
            ...
            ctx_dim=features - 1,
            d_model=cadgt_overrides.get(
                "hidden_dim", config["model_defaults"]["hidden_dim"]
            ),
        ...
        for x, y in train_loader:
            ...
            with torch.amp.autocast(device_type, enabled=use_amp):
                if model_name == "CADGT":
                    ...
                    loss = 0.25 * loss_fn(ps_short, y[:, :short_horizon, :]) + loss_fn(
                        pl_long, y
                    )

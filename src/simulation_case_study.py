...
def detect_congestion_window(true_speeds, node, length=100, threshold=40.0):
    ...
    for start in range(total_steps - length):
        window = true_speeds[start : start + length, 0, node]
...
def run_simulation(config_path="src/config.yaml"):
    ...
    scaler = StandardScaler(
        mean=checkpoint["scaler_mean"], std=checkpoint["scaler_std"]
    )
    logger.info(
        f"Loaded CADGT model. Scaler: mean={scaler.mean:.4f}, std={scaler.std:.4f}"
    )
    model.eval()
    ...
    start_step = detect_congestion_window(
        true_speeds, target_nodes[0], length=sim_length, threshold=35.0
    )
    logger.info(
        f"Selected simulation window starting at step {start_step} for {sim_length} steps."
    )
    ...
    sim_preds = scaler.inverse_transform(preds_scaled.reshape(-1, N)).reshape(
        sim_length, H, N
    )
    ...
    for node in target_nodes:
        ...
        plt.title(
            f"CADGT Horizon-Wise Drift Comparison - Node {node}",
            fontsize=16,
            fontweight="bold",
        )
        ...
        plt.plot(
            base_true, label="Ground Truth (5-min actual)", color="black", linewidth=3
        )
        ...
        for h_name, h_idx in horizons.items():
            ...
            if shift < sim_length:
                shifted_pred[shift:] = (
                    sim_preds[:-shift, h_idx, node]
                    if shift > 0
                    else sim_preds[:, h_idx, node]
                )
            ...
            plt.plot(
                shifted_pred,
                label=f"CADGT {h_name}",
                color=styles[h_name][0],
                linestyle=styles[h_name][1],
                linewidth=2,
                alpha=0.8,
            )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(out_dir, f"CADGT_horizon_drift_node_{node}.png"), dpi=300
        )
        plt.close()
        ...
        if is_congestion:
            ...
            plt.title(
                f"Congestion Response - Node {node} Focus",
                fontsize=14,
                fontweight="bold",
            )
            plt.plot(
                range(drop_start, drop_end),
                base_true[drop_start:drop_end],
                label="Ground Truth",
                color="black",
                linewidth=4,
            )
            for h_name in horizons.keys():
                plt.plot(
                    range(drop_start, drop_end),
                    aligned_preds[h_name][drop_start:drop_end],
                    label=f"CADGT {h_name}",
                    linewidth=2,
                    linestyle=styles[h_name][1],
                    color=styles[h_name][0],
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(out_dir, f"CADGT_congestion_zoom_node_{node}.png"), dpi=300
            )
            plt.close()
            ...
            for h_name in horizons.keys():
                ...
                delay_in_detection = max(0, pred_min_idx - min_speed_idx)
    ...
    if metrics:
        ...
        plt.title(
            "CADGT Error Growth Across Horizons (Congestion Window)",
            fontsize=14,
            fontweight="bold",
        )
